...
import { IDL, SplTokenStaking } from "../target/types/spl_token_staking";
...
describe("initialize-stake-pool", () => {
  ...
  // console.log("🚀 ~ describe ~ program:", program);
  console.log("initialize-stake-pool");
  ...
  console.log("🚀 ~ describe ~ keypair:", keypair);
  ...
  const programId = new anchor.web3.PublicKey(
    "Gd3eYJHnFieHnhfQBUZbv8ZBkPz85fenE77yBATnPDCX"
  );

  const program = new anchor.Program<SplTokenStaking>(IDL, programId, provider);
  console.log("🚀 ~ describe ~ program:", program);
  const tokenProgramInstance = splTokenProgram({
  ...
  it("StakePool initialized - happy path", async () => {
    ...
    const [stakePoolKey] = anchor.web3.PublicKey.findProgramAddressSync(
      [
        new Uint8Array(new anchor.BN(nonce).toArrayLike(Buffer, "le", 8)), // Ensure nonce is 8 bytes
        new Uint8Array(mintToBeStaked.toBuffer()),
        new Uint8Array(program.provider.publicKey.toBuffer()),
        new Uint8Array(Buffer.from("stakePool", "utf-8")),
      ],
      program.programId
    );
    console.log("🚀 ~ it ~ program.programId:", program.programId);
